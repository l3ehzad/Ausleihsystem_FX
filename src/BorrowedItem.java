...
/*
    //superseded with the one in main method:
...
*/
    ...
    //CONSTRUCTOR
    public BorrowedItem(int deviceID, String dshsID, String borrowDate, String reason) throws SQLException {
    ...
    public int getInventIDFromDeviceID(int deviceID) throws SQLException {
        int invID = 0;
            PreparedStatement statement = Main.con.prepareStatement("SELECT fk_inventID FROM device WHERE deviceID = "+ deviceID+";");
    ...
    public static int getPersonIDFromDshsID(String dshsID) throws SQLException{
        int personID = 0;
            ...
            PreparedStatement statement = Main.con.prepareStatement("SELECT personID FROM person WHERE dshsID = '"+ dshsID+"';");
            ResultSet result = statement.executeQuery();
            if (result.next())
    ...
    public void addBorrowedItemToSQL() throws Exception {
            PreparedStatement posted = Main.con.prepareStatement("INSERT INTO borroweditem ( borrowDate, reason, fk_inventID, fk_deviceID , fk_personID ) VALUES ('"+getBorrowDate()+"', '"+getReason()+"', '"+getInventID()+"', '"+getDeviceID()+"', '"+getPersonID()+"')");
            posted.executeUpdate();}

    public static boolean checkDeviceIdValidity(int deviceID) throws SQLException{
        ArrayList<Integer> array = new ArrayList<Integer>();

            PreparedStatement statement = Main.con.prepareStatement("SELECT deviceID FROM device;");
    ...
    private static void changeToUnavailable (int deviceID) throws SQLException{
            PreparedStatement posted = Main.con.prepareStatement("UPDATE device SET available = 0 WHERE deviceID = "+deviceID+";");
            posted.executeUpdate();}

    public static void changeToAvailable (int deviceID) throws SQLException {

            PreparedStatement posted = Main.con.prepareStatement("UPDATE device SET available = 1 WHERE deviceID = "+deviceID+";");
            posted.executeUpdate();}

    public static boolean checkDevAvailInBorrList(int devID) throws SQLException {
        ArrayList<Integer> array = new ArrayList<Integer>();

            PreparedStatement statement = Main.con.prepareStatement("SELECT fk_deviceID FROM borroweditem;");
    ...
    public static void deleteDeviceFromBorr(int devID) throws SQLException {
            PreparedStatement posted = Main.con.prepareStatement("DELETE FROM `borroweditem` WHERE `borroweditem`.`fk_deviceID` = "+ devID+";");
            posted.executeUpdate();
    }
    ...
    public static void deviceInfoBeforeRemove (int devID) throws SQLException {
        ...
        String labelName = null;
            ...
            PreparedStatement statement = Main.con.prepareStatement("SELECT deviceName, labelName FROM device WHERE deviceID = "+ devID+";");
    ...
    public static void listOfAvailItems () throws SQLException{
            PreparedStatement statement = Main.con.prepareStatement("SELECT * FROM `device` WHERE device.available = 1");
